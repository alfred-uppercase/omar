...
onMounted(async () => {
  try {
    const response = await axios.get('http://localhost/omar/api/get_categories');
      ...
      <div class="related-articles-content">
        <div v-for="category in categories" :key="category.id" :value="category.id" class="article">
          <div class="article-container"><img
            :src="`/omar/uploads/category_thumbnails/${category.thumbnail}`"
              loading="lazy" class="article-img">
              <div class="article-text">
                <h3 class="heading" dir="auto">{{ category.name }}</h3><a class="link stretched-link"
                :href="`/omar/search?search_string=&selected_city_id=&selected_category_id=${category.id}`"></a>
            </div>
          </div>
      </div>
    </div>
